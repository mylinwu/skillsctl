...
skillsctl 是一个本地 Agent Skills 管理工具，通过 CLI + TUI 的方式，把散落在 Claude Code、Codex、Cursor、OpenCode 等不同工具目录下的 skills 统一管理起来。它提供一个不会被 Agent 自动读取的本地技能仓库，并支持将技能通过 symlink、copy 或 Windows 友好链接方式派发到指定 Agent 的全局或项目目录中；关闭时删除派发链接，从而减少上下文污染和技能回收成本。
...
* 作为一个跨设备用户，我希望 skillsctl 同时支持 Windows 和 macOS，这样我可以在不同机器上使用一致的工作流。
...
* 作为一个开源用户，我希望可以在配置里添加自定义 Agent 路径映射，这样 skillsctl 可以适配我常用的 AI 工具。
...
* 仓库初始化：skillsctl 创建一个本地技能仓库目录，默认使用 \~/.skillsctl/repository，且配置默认存放于 \~/.skillsctl（例如配置文件为 \~/.skillsctl/config.json）。
...
* 派发记录：skillsctl 记录每一次受管派发，避免误删用户自己创建的本地 skills。
* 安全删除：skillsctl 只会自动删除自己创建或能安全验证的目标；其他情况必须明确询问用户。

### Agent 发现和配置（优先级：P0）

* 内置 Agent 映射：skillsctl 内置基于 `npx skills` 默认支持 Agents 的路径映射。
* 自定义 Agent 配置：用户可以在设置中新增或覆盖 Agent 定义，包括全局路径、项目路径、显示名称和派发方式。
* 自动扫描：skillsctl 可以扫描已知 Agent 的全局路径和项目 skill 路径，检测已有 skills。
* 手动配置路径：当自动扫描不完整或不准确时，用户可以手动配置具体 Agent 路径。

### CLI 交互（优先级：P0）

* `skillsctl init`：初始化配置和本地仓库。
* `skillsctl repo list`：列出本地仓库中的 skills。
* `skillsctl import <source>`：从远程或本地来源导入 skills。
* `skillsctl update [skill]`：从来源更新一个或多个已导入 skills。
* `skillsctl enable <skill> --agent <agent> --global|--project <path>`：派发一个 skill。
* `skillsctl disable <skill> --agent <agent> --global|--project <path>`：移除一个受管派发。
* `skillsctl app list`：列出已配置或已检测到的 Agents。
* `skillsctl app <agent> list`：展示某个 Agent 目标路径中可见的 skills。
* `skillsctl doctor`：检查 broken links、缺失路径、权限问题、重复名称和 Windows 链接能力。
* `skillsctl config`：查看或编辑关键设置。

### TUI 交互界面（优先级：P1）

* 启动 TUI：运行 `skillsctl` 或 `skillsctl manage` 打开交互式界面。
...
* 更新命令：skillsctl 参考 `npx skills update` 的行为更新本地仓库副本。
* copy 派发刷新：如果某个 skill 通过 copy 模式派发，skillsctl 可以检测已派发副本是否过期，并提供重新同步。
* symlink 派发刷新：如果通过 symlink 或 junction 派发，只要本地仓库副本更新，目标 Agent 目录会自动反映最新内容。

### App 本地 Skill 处理（优先级：P1）

* local-only 检测：skillsctl 识别 Agent 目标路径中存在、但不由 skillsctl 管理的 skills。
* local-only 展示：TUI 将这些 skills 标记为 `local only`，而不是当作普通仓库受管 skills。
* 导入本地 skill：用户可以把某个 app-local skill 导入到统一仓库。
* 冲突检测：当仓库 skill 和 Agent 本地 skill 共享相同目录名或 frontmatter name 时，skillsctl 需要给出警告。
...
* 用户从命令行安装或运行 skillsctl。
* 首次运行时，skillsctl 检测到没有配置，并引导用户初始化。
* 初始化流程询问：
  * 本地 skill 仓库应该放在哪里？
  * 默认派发方式是什么？
  * 要启用哪些已知 Agents？
...
* 用户运行 `skillsctl import vercel-labs/agent-skills --skill frontend-design`，或在 TUI 中选择导入。
* skillsctl 解析 source，发现可用的 `SKILL.md`，如果存在多个 skills，就让用户选择一个或多个。
* skillsctl 把选中的 skill 复制进本地仓库，并记录来源元数据。
...
* 用户选择一个 skill 并点击启用。
* skillsctl 询问 Agent 和作用域：全局或项目。
* 如果选择项目作用域，用户可以使用当前目录或输入项目路径。
* skillsctl 在缺失时创建所需目标文件夹。
* skillsctl 根据有效派发模式创建 symlink、junction 或 copy。
...
* 用户在 Agent 视图里关掉某个 skill，或运行 `skillsctl disable frontend-design --agent claude-code --global`。
* skillsctl 检查派发记录和目标路径。
...
* 用户运行 `skillsctl update`，或在 TUI 中选择更新。
* skillsctl 检查来源元数据并拉取最新版本。
* 对 symlink/junction 派发，无需额外动作。
* 对 copy 派发，skillsctl 标记已派发副本可能过期，并提供重新同步。
...
skillsctl 把所有 skills 先放进一个独立的本地仓库。这个仓库不会被任何 Agent 自动读取，只有当武明确打开开关时，skillsctl 才会把某个 skill 通过 symlink、junction 或 copy 派发到指定 Agent 的目标目录。武可以从仓库视角看到每个 skill 被哪些 Agent 使用，也可以从 Claude Code 或 Codex 视角看到当前可用和本地独有的 skills。

当任务结束时，武只需要在 TUI 里关掉开关，skillsctl 就会删除对应派发链接，但 skill 本体仍然安全地留在仓库里。这样，他既能快速复用技能，又能保持每个任务环境干净、可控。
...
* 首次初始化完成率：80% 以上用户可以不看文档完成 `skillsctl init`。
* 首个 skill 启用时间：从安装到成功派发第一个 skill 控制在 3 分钟以内。
* 清理信心：用户能够通过 `skillsctl doctor` 或 TUI 明确看到没有 broken links 或误删风险。
...
* 关键交付物：
  * 基于 source metadata 的 `skillsctl update`。
  * copy 派发过期检测和重新同步。
  * 更好的错误信息和确认流程。
