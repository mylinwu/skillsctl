...
* 配置目录固定为 `~/.skillctl`。
* 配置文件为 `~/.skillctl/config.json`。
* 本地仓库默认为 `~/.skillctl/repository`。
* 派发记录默认为 `~/.skillctl/deployments.json`。
...
配置文件: ~/.skillctl/config.json  
本地仓库: ~/.skillctl/repository  
派发记录: ~/.skillctl/deployments.json
...
* 仓库技能管理：管理 source of truth，也就是 `~/.skillctl/repository`。
...
本地路径:  
~/.skillctl/repository/frontend-design
...
* frontend-design -> ~/.skillctl/repository/frontend-design
...
将启用 2 个 skills:
* pr-review  
  创建: ~/.claude/skills/pr-review -> ~/.skillctl/repository/pr-review  
  方式: symlink
* api-debugger  
  创建: ~/.claude/skills/api-debugger -> ~/.skillctl/repository/api-debugger  
  方式: symlink
...
```bash
即将复制到:
\~/.skillctl/repository/old-commit-helper
...
指向目标:  
~/.skillctl/repository/legacy-review
...
通过:
* 配置文件存在: ~/.skillctl/config.json
* 本地仓库可读写: ~/.skillctl/repository
...
仓库版本:  
~/.skillctl/repository/product-manager
